# -*- coding: utf-8 -*-
"""
Created on Wed Jul  3 00:46:57 2019
...
def calculate_nfft(samplerate, winlen):
    ...
    the number of clean in a single window length.
    
    Having an FFT less than the window length loses precision by dropping
    many of the clean; a longer FFT than the window allows zero-padding
    of the FFT buffer which is neutral in terms of frequency domain conversion.
    :param samplerate: The sample rate of the signal we are working with, in Hz.
    :param winlen: The length of the analysis window in seconds.
    """
    window_length_clean = winlen * samplerate
    nfft = 1
    while nfft < window_length_clean:
        nfft *= 2
    return nfft
...
if len(os.listdir('clean')) == 0:
    for f in tqdm(df.fname):
        ...
        mask=envelope(signal,rate,0.05)
